						...
						<table class="table table-hover table-bordered table-striped" id="supplier_table">
							...
							<tbody>
								<?php
									...
									$query = mysqli_query($con,"SELECT * FROM `ordertable` INNER JOIN `team` ON `ordertable`.team_id = `team`.team_id INNER JOIN `customertable` ON `ordertable`.c_id = `customertable`.c_id INNER JOIN `inventory` ON `ordertable`.item_id = `inventory`.item_id INNER JOIN `payment_reason` ON `ordertable`.ORno = `payment_reason`.order_id WHERE `payment_reason`.partbilling = 'Approve' AND `payment_reason`.user_role = '6' AND `ordertable`.username = '".$_SESSION['username']."' ") or die(mysqli_error($con));
							...
							<tr class="record">
  														...
  														<div class="modal-dialog">
    														<div class="modal-content">
      															...
      															<div class="modal-body">
        															...
        															<p style="color:red;">Rejected By: '.$row['username'].'</p>
								...
								<td>
									<div class="btn-group">
										<a href="generate_bill.php?order_id=<?php echo $row[0]; ?>" data-toggle="tooltip" title="View Orders" class="btn btn-primary btn-xs">Generate Invoice</a>
									</div>
								</td>
							</tr>
							<?php } ?>
